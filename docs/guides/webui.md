# CentaurAI WebUI Mode - Startup Guide

CentaurAI supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts CentaurAI with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use CentaurAI from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\CentaurAI\CentaurAI.exe" --webui

# Or if CentaurAI is in your PATH
CentaurAI.exe --webui
```
...
```bash
# Using full path
/Applications/CentaurAI.app/Contents/MacOS/CentaurAI --webui

# Or using open command
open -a CentaurAI --args --webui
```

### Method 2: Create Shell Script

Create `start-centaurai-webui.sh`:
...
```bash
chmod +x start-centaurai-webui.sh
./start-centaurai-webui.sh
```
...
chmod +x CentaurAI-*.AppImage

# Run with --webui flag
./CentaurAI-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/centaurai-webui.desktop`:

```ini
[Desktop Entry]
Name=CentaurAI WebUI
Comment=Start CentaurAI in WebUI mode
Exec=/opt/CentaurAI/centaurai --webui
Icon=centaurai
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/centaurai-webui.desktop
```
...
Create `~/bin/start-centaurai-webui.sh`:
...
```bash
chmod +x ~/bin/start-centaurai-webui.sh
```
...
Create `/etc/systemd/system/centaurai-webui.service`:

```ini
[Unit]
Description=CentaurAI WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/CentaurAI/centaurai --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable centaurai-webui.service
sudo systemctl start centaurai-webui.service

# Check status
sudo systemctl status centaurai-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run CentaurAI in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making CentaurAI accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running CentaurAI WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/finewood2008/centaurai-team/issues/217)
...
#### 4. Download and Install CentaurAI

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/finewood2008/centaurai-team/releases
wget https://github.com/finewood2008/centaurai-team/releases/download/vVERSION/CentaurAI_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/finewood2008/centaurai-team/releases/download/vVERSION/CentaurAI_VERSION_arm64.deb

# Install the package
apt install -y ./CentaurAI_*.deb

# Verify installation
which CentaurAI
```

#### 5. Launch CentaurAI WebUI

```bash
# Start CentaurAI in WebUI mode with no-sandbox flag
CentaurAI --no-sandbox --webui
```
...
To access CentaurAI from other devices on your local network:

```bash
# Start with --remote flag
CentaurAI --no-sandbox --webui --remote
...
CentaurAI --no-sandbox --webui --port 8080
```
...
chmod +x /opt/CentaurAI/centaurai
```

#### Out of Memory

CentaurAI requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if CentaurAI is running: look for "Server started" message
...
4. **Keep device charged** - Running CentaurAI consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **CentaurAI Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch CentaurAI quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-centaurai.sh << 'EOF'
#!/bin/bash
echo "Starting CentaurAI WebUI..."
CentaurAI --no-sandbox --webui --remote
EOF

# Make executable
chmod +x ~/start-centaurai.sh
...
```bash
proot-distro login ubuntu -- bash -c "CentaurAI --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/finewood2008/centaurai-team/issues/217)
...
```cmd
CentaurAI.exe --webui --remote
```

### macOS

```bash
/Applications/CentaurAI.app/Contents/MacOS/CentaurAI --webui --remote
```

### Linux

```bash
centaurai --webui --remote
```
...
netsh advfirewall firewall add rule name="CentaurAI WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add CentaurAI
...
which centaurai
# or
find /opt -name "centaurai" 2>/dev/null
```
...
```powershell
& "C:\Program Files\CentaurAI\CentaurAI.exe" --webui 2>&1 | Tee-Object -FilePath centaurai.log
```

**macOS/Linux:**

```bash
/path/to/centaurai --webui 2>&1 | tee centaurai.log
```
...
export CENTAURAI_PORT=8080

# Allow remote access without passing --remote
export CENTAURAI_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as CENTAURAI_ALLOW_REMOTE=true)
export CENTAURAI_HOST=0.0.0.0

# Then start the application
centaurai --webui

# You can also pass the port directly via CLI
centaurai --webui --port 8080
```
...
```cmd
# Using full path
"C:\Program Files\CentaurAI\CentaurAI.exe" --resetpass

# Or for a specific user
"C:\Program Files\CentaurAI\CentaurAI.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/CentaurAI.app/Contents/MacOS/CentaurAI --resetpass

# Or for a specific user
/Applications/CentaurAI.app/Contents/MacOS/CentaurAI --resetpass username
```
...
# Or for a specific user
centaurai --resetpass username

# Or using full path
/opt/CentaurAI/centaurai --resetpass
```
...
- [GitHub Issues](https://github.com/finewood2008/centaurai-team/issues)
...
2. Search [existing issues](https://github.com/finewood2008/centaurai-team/issues)
3. Create a [new issue](https://github.com/finewood2008/centaurai-team/issues/new) with:
   - Your OS and version
   - CentaurAI version
   - Steps to reproduce
...
**Happy using CentaurAI in WebUI mode!** 🚀
