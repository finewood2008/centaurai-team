...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'centaurai');
};
 ...
 * New installs use ~/.centaurai; existing AionUi paths remain supported.
 * 获取数据目录路径，macOS 上使用符号链接。
 * 新安装使用 ~/.centaurai；已有 AionUi 路径继续兼容。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const canonicalPath = path.join(rootPath, 'centaurai');
  const legacyPath = path.join(rootPath, 'aionui');
  const useLegacyPath = !existsSync(canonicalPath) && existsSync(legacyPath);
  const dataPath = useLegacyPath ? legacyPath : canonicalPath;
  const symlinkName = getEnvAwareName(useLegacyPath ? '.aionui' : '.centaurai');
  return ensureCliSafeSymlink(dataPath, symlinkName);
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.centaurai-config; legacy symlinks remain valid.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.centaurai-config，旧版符号链接继续有效。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.centaurai-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[CentaurAI] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[CentaurAI] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[CentaurAI] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[CentaurAI] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
