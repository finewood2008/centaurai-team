...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[CentaurAI] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[CentaurAI] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[CentaurAI] 数据迁移失败:', error);
  }
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[CentaurAI] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[CentaurAI:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[CentaurAI] Failed to ensure assistant dirs:', error);
  }
