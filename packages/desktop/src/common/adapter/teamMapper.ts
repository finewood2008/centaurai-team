...
export function fromBackendAgent(raw: unknown): TeamAgent {
  ...
  return {
    ...
    custom_agent_id: (r.assistant_id as string | undefined) ?? (r.custom_agent_id as string | undefined),
...
export function toBackendAgent(a: Omit<TeamAgent, 'slot_id' | 'conversation_id'>): Record<string, unknown> {
  const assistantId = a.custom_agent_id?.trim();
  if (!assistantId) throw new Error('Team agent assistant_id is required');
  return {
    name: a.agent_name,
