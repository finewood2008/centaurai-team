...
const tarballName = `centaurai-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
const tarballContentDir = path.join(stagingDir, 'centaurai-web');
...
const executableName = platform === 'win32' ? 'centaurai-web.exe' : 'centaurai-web';
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} centaurai-web`, {
