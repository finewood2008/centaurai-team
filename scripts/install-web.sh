...
# ============================================================================
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/finewood2008/centaurai-team/main/scripts/install-web.sh | bash
#   # Or specify version:
#   VERSION=1.0.0 bash install-web.sh
#   # Or install to custom directory:
#   INSTALL_DIR=/opt/centaurai-web bash install-web.sh
# ============================================================================
...
INSTALL_DIR="${INSTALL_DIR:-${HOME}/.local/share/centaurai-web}"
BIN_DIR="${BIN_DIR:-${HOME}/.local/bin}"
MIRROR="${MIRROR:-https://github.com/finewood2008/centaurai-team/releases/download}"
...
Options:
  ...
  --install-dir <path>      Specify installation directory (default: ~/.local/share/centaurai-web)
...
detect_platform_arch() {
    ...
    TARBALL_NAME="centaurai-web-${VERSION}-${PLATFORM}-${ARCH}.tar.gz"
...
resolve_version() {
    ...
    if [[ "$VERSION" == "latest" || "$VERSION" =~ [a-zA-Z_] ]]; then
        ...
        if command -v curl &>/dev/null; then
            VERSION=$(curl -fsSL "https://api.github.com/repos/finewood2008/centaurai-team/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        elif command -v wget &>/dev/null; then
            VERSION=$(wget -qO- "https://api.github.com/repos/finewood2008/centaurai-team/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        else
...
download_tarball() {
    ...
    # Build download URL
    # MIRROR formats:
    #   - GitHub: https://github.com/finewood2008/centaurai-team/releases/download
...
extract_tarball() {
    ...
    # Extract tarball
    # Tarball root directory is centaurai-web/, rename after extraction to INSTALL_DIR
    ...
    if [[ -d "${extract_temp}/centaurai-web" ]]; then
        mv "${extract_temp}/centaurai-web" "$INSTALL_DIR"
    else
        die "Tarball structure is invalid (missing centaurai-web/ directory)"
    fi

    success "Extracted to $INSTALL_DIR"

    # Set executable permission on the bun-compiled standalone binary
    chmod +x "${INSTALL_DIR}/centaurai-web" 2>/dev/null || true
    ...
    # Verify installation
    if [[ ! -x "${INSTALL_DIR}/centaurai-web" ]]; then
        die "Installation failed: ${INSTALL_DIR}/centaurai-web not found or not executable"
    fi
...
create_symlink() {
    local symlink_path="${BIN_DIR}/centaurai-web"
    local legacy_symlink_path="${BIN_DIR}/aionui-web"
    local target_path="${INSTALL_DIR}/centaurai-web"
    ...
    # Keep the old command working for existing automation without advertising
    # it as the primary CentaurAI command.
    if [[ -L "$legacy_symlink_path" ]]; then
        rm "$legacy_symlink_path"
    fi
    if [[ ! -e "$legacy_symlink_path" ]]; then
        ln -s "$target_path" "$legacy_symlink_path"
    fi
...
update_shell_profile() {
    ...
    # Add to profile
    echo "" >> "$profile_file"
    echo "# Added by centaurai-web installer" >> "$profile_file"
...
print_summary() {
    ...
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo -e "  ${BOLD}📍 Symlink:${NC}                ${BIN_DIR}/centaurai-web"
    fi
    echo ""
    ...
    if [[ "$CREATE_SYMLINK" == "1" && ":$PATH:" == *":${BIN_DIR}:"* ]]; then
        ...
        echo "    centaurai-web start"
        echo ""
        echo "    # Check version"
        echo "    centaurai-web version"
    else
        echo "    # Start CentaurAI WebUI (using full path)"
        echo "    ${INSTALL_DIR}/centaurai-web start"
        echo ""
        echo "    # Or add symlink to PATH:"
        if [[ "$CREATE_SYMLINK" == "1" ]]; then
            echo "    export PATH=\"${BIN_DIR}:\$PATH\""
        else
            echo "    ln -s ${INSTALL_DIR}/centaurai-web ~/.local/bin/centaurai-web"
            echo "    export PATH=\"~/.local/bin:\$PATH\""
        fi
    fi
    echo ""
    echo -e "  ${BOLD}📖 Documentation:${NC}  https://github.com/finewood2008/centaurai-team"
    echo -e "  ${BOLD}🐛 Report issues:${NC}  https://github.com/finewood2008/centaurai-team/issues"
    echo ""
