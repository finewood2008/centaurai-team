...
export INSTALL_DIR="/tmp/centaurai-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/centaurai-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/centaurai-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/centaurai-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/centaurai-web"
    exit 1
fi
...
VERSION_OUTPUT=$(centaurai-web version 2>&1 || echo "")
